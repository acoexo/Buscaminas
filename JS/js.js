...
function renderizarTablero() {
  ...
  for (let i = 0; i < filas; i++) {
    ...
    for (let j = 0; j < columnas; j++) {
      ...
      celda.style.backgroundImage = 'url("./../IMG/hide.png")';
...
function mostrarMensaje(event) {
  ...
  if (!celda.mostrado && !celda.marcada) {
    ...
    if (celda.valor === 1) {
      ...
      event.target.style.backgroundImage = `url("./../IMG/${minesCount}.png")`;
    }
...
function marcarCasilla(event) {
  ...
  if (!celda.mostrado) {
    if (!celda.marcada) {
      event.target.style.backgroundImage = 'url("./../IMG/bandera.png")';
      celda.marcada = true;
    } else {
      event.target.style.backgroundImage = 'url("./../IMG/hide.png")';
      celda.marcada = false;
    }
  }
}
